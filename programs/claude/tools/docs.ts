// Docs are read from the working tree next to this file, so the index and
// pages are always the ones checked in here, with no build or install step.
const DOCS_DIR = new URL("../docs/", import.meta.url);

/** Doc names are paths under docs/ without the `.md` suffix, e.g. `claude-tools/gh/get-release`. */
export async function listDocs(dir: URL = DOCS_DIR): Promise<string[]> {
  const names: string[] = [];
...
export async function renderIndex(dir: URL = DOCS_DIR): Promise<string> {
  ...
  return [
    "Read one with `claude-tools docs <name>`; any unique trailing part of a name works.",
    "",
    ...rows,
  ].join("\n");
}
...
export async function main(): Promise<void> {
  // Deno.args[0] is "docs"; the rest name a doc, either as one path or as
  // separate words (`docs gh get-release`), which spell the same thing.
  const query = Deno.args.slice(1).join("/");
  if (!query) {
    console.log(await renderIndex());
    return;
  }

  const resolution = resolveDoc(await listDocs(), query);
  switch (resolution.kind) {
    case "found":
      console.log(await Deno.readTextFile(new URL(`${resolution.name}.md`, DOCS_DIR)));
      return;
    case "ambiguous":
      ...
      break;
    case "missing":
      console.error(`No doc named "${query}". Run \`claude-tools docs\` to list them.`);
      Deno.exit(1);
  }
}
