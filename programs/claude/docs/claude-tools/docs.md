...
Print the documentation for `claude-tools` commands. Without a name it prints a one-line-per-doc index; with a name it prints that doc as Markdown, so an agent reads only what it needs. `claude-hooks docs` does the same for the hooks.
...
Docs are read from `programs/claude/docs/claude-tools/` in the dotfiles working tree, so a page is available as soon as its file exists.
